...
### I. 运行阶段：I2C 信号质量与传感器不响应 (AHT20/BMP280)

- **现象**: 即使硬件接线正确且地址扫描正常，读取数据时仍出现 `AcknowledgeCheckFailed` 或 `InvalidCrc`。
- **事实澄清**: 经过深度排查，**硬件并未损坏或改动**。问题的关键在于：
  1. **线上库不兼容**: 第三方 Crates.io 库（如 `embedded-aht20`）在 `esp-hal 1.0` 的 I2C 实现下存在微小的时序偏差。
  2. **协议死锁**: AHT20 对 `write_read` 这一复合操作极度敏感，使用该方法读取状态位会导致传感器底层状态机挂起，返回错误的 CRC。
- **最终对策**:
  - **废弃线上通用库**: 为了追求极致稳定性，停止使用 `embedded-aht20` 和 `bme280` 库。
  - **实现本地闭环驱动**: 参考本项目中的 `aht20.rs` 和 `bmp280.rs`。通过直接操作 I2C 的单步读取指令，并手动实现官方补偿算法，绕过了一切不确定的中间层逻辑。
  - **硬件加固**: 虽然核心问题在软件，但仍建议外接上拉电阻以确保信号上升沿足够陡峭，这是 I2C 总线长久稳定的物理基础。
